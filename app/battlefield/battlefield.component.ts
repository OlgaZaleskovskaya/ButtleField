...
export class BattlefieldComponent implements OnInit, OnChanges {
  ...
  setBorder(cell, y, x): string {
    ...
    if (cell === 0 || !this.my) {
      result = 'empty ' + result;
      if (cell === 0 && Point.pointIn(new Point(x, y), this.shooted)) {
        result += 'shooted';
      }
    } else {
  ...
  clickShot(x, y) {
    if (this.gameOver || this.shootBlock || Point.pointIn(new Point(x, y), this.shooted)) {return; }
    ...
    if (!this.my) {
      ...
      setTimeout(
        () => {
          this.shooted.push(new Point(x, y));
          this.mover.emit('my');
